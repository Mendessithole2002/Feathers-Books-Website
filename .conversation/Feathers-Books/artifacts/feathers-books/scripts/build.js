...
let metroPort = Number(process.env.EXPO_BUILD_PORT || 8081);
...
async function selectMetroPort() {
  if (process.env.EXPO_BUILD_PORT) {
    if (!(await isPortAvailable(metroPort))) {
      throw new Error(`EXPO_BUILD_PORT ${metroPort} is already in use`);
    }
    return;
  }
  ...
  throw new Error('Could not find an available Metro build port between 8081 and 8099');
}
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  const env = {
    ...
    EXPO_PUBLIC_ADMIN_PASSWORD: process.env.ADMIN_PASSWORD || '',
  };
