...
function Sidebar() {
  ...
  return <aside className="desktop-sidebar">
    ...
    <div className="mt-auto">
      ...
      <Link href={admin ? '/admin/new' : '/admin/sign-in'} className={`side-link ${location.startsWith('/admin') ? 'active' : ''}`} data-testid="link-admin"><Shield size={17} />Publisher studio</Link>
...
function MobileNav() {
  ...
  return <nav className="mobile-nav" aria-label="Mobile navigation">
    ...
    <Link href="/admin/sign-in" className={location.startsWith('/admin') ? 'active' : ''} data-testid="mobile-admin"><Shield size={19} /><span>Studio</span></Link>
  </nav>;
}
...
function AdminSignIn() {
  ...
  useEffect(() => { if (admin) navigate('/admin/new'); }, [admin, navigate]);
  ...
  return <div className="min-h-[100dvh] bg-[hsl(var(--primary))] px-5 py-8 text-[hsl(var(--primary-foreground))]"><div className="mx-auto flex min-h-[calc(100dvh-64px)] w-full max-w-[520px] flex-col justify-center"><div className="mb-16"><Brand /></div><div className="eyebrow mb-4">Publisher studio</div><h1 className="serif m-0 text-6xl leading-none">Make room for<br /><em className="text-[hsl(var(--accent))]">good stories.</em></h1><p className="mt-6 max-w-md text-sm leading-6 opacity-65">Reader browsing is public. Enter the publisher key to manage the Feathers Books catalog.</p><form onSubmit={submit} className="mt-10"><label className="mb-2 block text-xs font-bold">Publisher key</label><input type="password" value={password} onChange={(event) => { setPassword(event.target.value); setError(''); }} className="h-12 w-full rounded-xl border border-white/20 bg-white/10 px-4 outline-0 placeholder:text-white/35 focus:border-[hsl(var(--accent))]" placeholder="Enter your studio key" autoComplete="current-password" data-testid="input-admin-password" />{error && <p className="mt-3 text-sm text-[#e99a8d]" role="alert" data-testid="status-admin-error">{error}</p>}<button className="button mt-5 w-full bg-[hsl(var(--accent))] text-[hsl(var(--primary))]" disabled={!password} data-testid="button-admin-submit"><Lock size={16} />Open Publisher Studio</button></form><button className="mt-6 inline-flex items-center gap-2 text-sm opacity-65 hover:opacity-100" onClick={() => navigate('/')} data-testid="button-admin-back"><ChevronLeft size={16} />Back to reader browsing</button><p className="mt-12 text-xs opacity-40">Demo publisher key: feathers</p></div></div>;
}
...
function AdminNew() {
  ...
  const save = (event: React.FormEvent) => { event.preventDefault(); if (!title.trim() || !author.trim() || !description.trim() || !content.trim() || !pages) { setError('Add a title, author, description, page count, and manuscript before publishing.'); return; } addBook({ title: title.trim(), author: author.trim(), category, description: description.trim(), content: content.trim(), pages: Number(pages), price: Number(price) || 0, cover: 'lighthouse', coverUri, published }); setToast('Book added to the catalog'); setTitle(''); setAuthor(''); setDescription(''); setContent(''); setPages(''); setPrice(''); setCoverUri(''); };
  ...
  if (!admin) return null;
  return <Shell><div className="content-frame"><div className="mb-8 flex items-end justify-between gap-4"><div><div className="eyebrow mb-3">Publisher studio</div><h1 className="serif m-0 text-6xl leading-none">New book</h1><p className="muted mt-3 max-w-md text-sm leading-6">Bring a new story into the Feathers Books catalog.</p></div><button className="button button-outline" onClick={() => { localStorage.removeItem(STORAGE_KEY); window.location.href = '/'; }} data-testid="button-studio-signout"><LogOut size={15} />Sign out</button></div>
    <form className="admin-card" onSubmit={save}><div className="flex items-center justify-between"><h2 className="serif m-0 text-3xl">Edition details</h2><span className="rounded-full bg-[hsl(var(--secondary))] px-3 py-1 text-xs font-bold">New release</span></div>{error && <div className="mt-5 rounded-lg bg-[#f5d9d2] px-4 py-3 text-sm text-[#8a3f37]" role="alert" data-testid="status-book-error">{error}</div>}<div className="field"><label htmlFor="book-title">Title</label><input id="book-title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="The Glass Orchard" data-testid="input-book-title" /></div><div className="field"><label htmlFor="book-author">Author</label><input id="book-author" value={author} onChange={(event) => setAuthor(event.target.value)} placeholder="Author name" data-testid="input-book-author" /></div><div className="field"><label htmlFor="book-category">Category</label><select id="book-category" value={category} onChange={(event) => setCategory(event.target.value)} data-testid="select-book-category">{categories.slice(1).map((item) => <option key={item}>{item}</option>)}</select></div><div className="field"><label htmlFor="book-description">Description</label><textarea id="book-description" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="What will readers discover?" data-testid="input-book-description" /></div><div className="grid gap-3 sm:grid-cols-2"><div className="field"><label htmlFor="book-pages">Pages</label><input id="book-pages" type="number" min="1" value={pages} onChange={(event) => setPages(event.target.value)} placeholder="240" data-testid="input-book-pages" /></div><div className="field"><label htmlFor="book-price">Price (USD)</label><input id="book-price" type="number" min="0" step=".01" value={price} onChange={(event) => setPrice(event.target.value)} placeholder="9.99" data-testid="input-book-price" /></div></div><div className="field"><label htmlFor="book-cover">Cover artwork</label><label className="flex min-h-24 cursor-pointer items-center justify-center gap-3 rounded-xl border border-dashed border-[hsl(var(--border))] bg-[hsl(var(--secondary))] p-4 text-sm hover:border-[hsl(var(--accent))]">{coverUri ? <img src={coverUri} alt="Selected cover preview" className="h-20 w-14 rounded object-cover" /> : <ImageIcon className="text-[hsl(var(--accent))]" />}<span>{coverUri ? 'Cover selected · choose another' : 'Choose a cover image from your device'}</span><input id="book-cover" type="file" accept="image/*" className="sr-only" onChange={chooseCover} data-testid="input-book-cover" /></label></div><div className="field"><label htmlFor="book-content">Book manuscript</label><textarea id="book-content" className="!min-h-48" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Paste the opening text or full manuscript here..." data-testid="input-book-content" /></div><label className="mt-5 flex cursor-pointer items-center gap-3 text-sm"><input type="checkbox" checked={published} onChange={(event) => setPublished(event.target.checked)} className="h-4 w-4 accent-[hsl(var(--accent))]" data-testid="input-book-published" />Publish immediately</label><button className="button button-primary mt-7 w-full" data-testid="button-publish-book"><Send size={16} />Publish book</button></form>{toast && <Toast message={toast} onDone={() => setToast('')} />}<AdminCatalog /></div></Shell>;
}
...
function Router() {
  return <Switch><Route path="/" component={Discover} /><Route path="/library" component={LibraryPage} /><Route path="/book/:id" component={BookDetail} /><Route path="/reader/:id" component={Reader} /><Route path="/admin/sign-in" component={AdminSignIn} /><Route path="/admin/new" component={AdminNew} /><Route component={NotFound} /></Switch>;
}
